...
# invoke the function
peer chaincode query -C bcereconchannel -n basicts -c '{"Args":["org.hyperledger.fabric:GetMetadata"]}' | jq
```
...
```bash
    docker run --rm -d --name peer0org1_assettx_ccaas  \
                  --network subex_blockchain_network \
...
```bash
 docker run --rm -it -p 9229:9229 --name peer0org2_basic_ccaas --network subex_blockchain_network -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 -e CORE_CHAINCODE_ID_NAME=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 basic_ccaas_image:latest
```

Java for example, could be started like this

```bash
 docker run --rm -it --name peer0org1_basicj_ccaas -p 8000:8000 --network subex_blockchain_network -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basicj_1.0:b014a03d8eb1898535e25b4dfeeb3f8244c9f07d91a06aec03e2d19174c45e4f -e CORE_CHAINCODE_ID_NAME=basicj_1.0:b014a03d8e
